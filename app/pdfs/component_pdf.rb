...
class ComponentPdf < Prawn::Document
  def initialize(componentdetail, compdetail, uRl)
...
move_down 5
text "<b>Component List </b>", :size => 12, :align => :center, :inline_format => true
...
bounding_box([bounds.left, bounds.top - 80], width: bounds.width) do
  ...
  data2   = []
  data2 = ([
    [{:content => "S.No. "},  {:content => "Component Code"}, {:content => "Description"}],
  
  ])
  @componentdetail.each do |component|
      data2 += ([
        [{:content => count.to_s},  {:content => component.compt_code.to_s }, {:content => component.compt_descp.to_s}],
  ...
  style column(1), :width     => 140
  style column(2), :width     => 350
  style column(0..2),:inline_format => :true, :size => 10
  # cells.padding =4
  self.header = true
