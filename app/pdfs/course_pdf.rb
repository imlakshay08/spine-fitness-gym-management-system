...
class CoursePdf < Prawn::Document
  def initialize(coursedetail, compdetail, uRl)
...
move_down 5
text "<b>Course List </b>", :size => 12, :align => :center, :inline_format => true
...
bounding_box([bounds.left, bounds.top - 80], width: bounds.width) do
  ...
  data2   = []
  data2 = ([
    [{:content => "S.No. "},  {:content => "Course Detail"}, {:content => "Description"},{:content => "Duration of Course"},{:content => "Total Seats"}],
  
  ])
  @coursedetail.each do |course|
      data2 += ([
        [{:content => count.to_s},  {:content => course.crse_code.to_s }, {:content => course.crse_descp.to_s}, {:content => course.crse_duration.to_s}, {:content => course.crse_seats.to_s}],
  ...
  style column(3), :width     => 120
  style column(4), :width     => 100
