...
class CategoryPdf < Prawn::Document
  def initialize(categorydetail, compdetail, uRl)
...
move_down 5
text "<b>Category List </b>", :size => 12, :align => :center, :inline_format => true
...
bounding_box([bounds.left, bounds.top - 80], width: bounds.width) do
  ...
  data2   = []
  data2 = ([
    [{:content => "S.No. "},  {:content => "Category"}, {:content => "Description"}],
  
  ])
  @categorydetail.each do |category|
      data2 += ([
        [{:content => count.to_s},  {:content => category.cat_code.to_s }, {:content => category.cat_descp.to_s}],
