...
class HousePdf < Prawn::Document
  def initialize(housedetail, compdetail, uRl)
...
move_down 5
text "<b>Club List </b>", :size => 12, :align => :center, :inline_format => true
...
bounding_box([bounds.left, bounds.top - 80], width: bounds.width) do
  ...
  data2   = []
  data2 = ([
    [{:content => "S.No. "},  {:content => "Club Name"}],
  
  ])
  @housedetail.each do |house|
      data2 += ([
        [{:content => count.to_s},  {:content => house.hs_house_name.to_s }],
  ...
  style column(1), :width     => 480

  style column(0..1),:inline_format => :true, :size => 10
  # cells.padding =4
  self.header = true
