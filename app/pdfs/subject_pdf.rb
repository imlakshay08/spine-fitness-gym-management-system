...
class SubjectPdf < Prawn::Document
    def initialize(subjectdetail, compdetail, uRl)
  ...
  move_down 5
  text "<b>Subject List </b>", :size => 12, :align => :center, :inline_format => true
  ...
  bounding_box([bounds.left, bounds.top - 80], width: bounds.width) do
    ...
    data2   = []
    data2 = ([
      [{:content => "S.No. "},  {:content =>"Course Code\nCourse Name"}, {:content => "Subject Code\nSubject Name"}, {:content => "Type"}, {:content => "Active"}],
    
    ])

    @subjectdetail.each do |subject| 
       
      data2 += ([
        [{:content => count.to_s}, {:content => subject.coursecode.to_s+"\n"+subject.coursename.to_s }, {:content => subject.sub_code.to_s+"\n"+subject.sub_name.to_s }, {:content => subject.sub_type.to_s}, {:content => subject.sub_activ.to_s}],
      ])
    end
    table([] + data2, :width => 530)do
    style row(0),:font_style=>:bold,:background_color => 'e7e7e7'
    style column(0), :width     => 40
    style column(1), :width     => 190
    style column(2), :width     => 200
    style column(3), :width     => 50
    style column(4), :width     => 50
    style column(0..4),:inline_format => :true, :size => 10
    self.header = true
    # cells.padding =4
