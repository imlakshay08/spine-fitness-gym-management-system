...
class FeeImportController < ApplicationController
    ...
    helper_method :currency_formatted,:year_month_days_formatted,:formatted_date,:set_ent,:set_dct  
      ...
      def ajax_process
        ...
        if params[:identity]!=nil && params[:identity]!='' && params[:identity] == 'FEEIMPORT'
          fee_import_process() 
           return
        end
      end

      def fee_import_process
        @compcodes = session[:loggedUserCompCode]
        file = params[:file]
        ...
        if file.present?
          ...
          vhtml = render_to_string partial: 'fee_import/view_fee_import', locals: { mydata: csv_data }
          respond_to do |format|
            format.json { render json: { data: vhtml, status: true } }
          end
        else
          respond_to do |format|
            format.json { render json: { data: "No data found.", status: false } }
          end
        end
      end
      ...
      def create
              ...
              $isimport = params[:process_file_data].to_s
                      ...
                      if params[:process_file_data].to_s!=nil && params[:process_file_data].to_s!='' && params[:process_file_data].to_s =='employee'
                        ...
                        if MstStudent.import(params[:file])
                            if params[:checkscale_updated] && params[:checkscale_updated].to_s == 'Y'
                                flash[:error] =  "Scale updated successfully "+(($xcount.to_s.length.to_i >0 ) ? $xcount.to_s : '0' )+" record(s)!"
                            else
         ...
         redirect_to "#{root_url}"+"fee_import"
        end
