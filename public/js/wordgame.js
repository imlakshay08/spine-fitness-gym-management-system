...
(function () {
  ...
  $(function () {
    var words = ['Club', 'Clove', 'Mint', 'American'];
    ...
    var puzzle = wordfind.newPuzzle(
      words, 
      {height: 8, width:7, fillBlanks: true}
    );
    wordfind.print(puzzle);
  });
  
  document.addEventListener('DOMContentLoaded', function() {
    // Disable scrolling for the puzzle section
    document.getElementById('puzzle').classList.add('puzzle-no-scroll');

    // Prevent scrolling on touch devices within the puzzle section
    document.getElementById('puzzle').addEventListener('touchmove', function(event) {
        event.preventDefault();
    }, { passive: false });

    // Prevent default action for clicks within the puzzle section
